...
Bashship is highly customizable through environment variables. You can set these variables by creating a `~/.config/bashship.conf` file. Below are the available configuration options:

| Variable               | Description                                          | Common Values               |
|------------------------|------------------------------------------------------|-----------------------------|
| `PROMPT_SYMBOL`        | The symbol used at the end of the prompt             | `>`, `$`, `#`               |
| `PROMPT_ORDER`         | The order of prompt components                       | `("user" "host" "path" "git-branch" "git-status" "time" "jobs" "container")` |
